...
for data,labels in tqdm(val_loader):
	...
	for i in range(len(labels)):
		if labels[i] == torch.argmax(output_clean[i]):
			soft = softmax(output_clean[i].cpu().detach().numpy())
			for j in range(0, 20):
				if np.sort(soft)[-2] < j / 20:
					range_confidence[j] +=1
					break
	accuracy_list.append(acc_clean)
	
print("Test accuracy:",np.sum(accuracy_list)/len(val_dataset))
print(range_confidence)
